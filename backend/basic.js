...
for (var i = 0; i < number.length; i++) {
  number[i].addEventListener("click", function(e) {
    ...
    if (resultDisplayed === false) {
      ...
      (resultDisplayed === true && lastChar === "+" || lastChar === "-" || lastChar === "×" || lastChar === "÷" || lastChar === "%") {
...
result.addEventListener("click", function() {
  ...
  // first divide, then multiply, then subtraction, then addition, and then remainder. 
  ...
  // for 5/20/24 function calculateModulus() { }
  var mod = operators.indexOf("%");
  while (mod != -1) {
    numbers.splice(mod, 2, parseFloat(numbers[mod]) % parseFloat(numbers[mod + 1]));
